...
public class ActionService {
    ...
    private final static String NONE_ENTITY = "-1";
    /*
    约定格式如下：
    board_id    b_(int)
    user_id     (int)
    ...
    public String handlerJsonCode(String code) throws SQLException{
        ...
        String cardId=null,laneId=null,content=null,description=null;
        ...
        if (entity.length() != 0 && entity.equals(ENTITY_TYPE_CARD)) {
            Card card = dohandlerCard(Integer.parseInt(cardId.substring(2)), Integer.parseInt(userId), Integer.parseInt(laneId.substring(2)), action, content, description);
    ...
    public Lane doHandlerLane(int laneId , int userId, int boardId, String action, String content, String description)throws SQLException{
        switch (action){
            case REQUEST_INSERT:
                if(description == null) description=DEFAULT_LANE_DESCRIPTION;
    ...
    public Board doHandlerBoard(int boardId , int userId, String action, String description)throws SQLException{
        switch (action){
            case REQUEST_INSERT:
                if(description == null) description=DEFAULT_BOARD_DESCRIPTION;
